...
G = generate_synthetic_large_graph(10000, 50000, 10)

# Print 50 edges, their base weights, and the nodes they join
for i, (u, v) in enumerate(random.sample(list(G.edges()), 50)):
    base_weight = G[u][v]['base_weight']
    node_u_weight = G.nodes[u]['weight']
    node_v_weight = G.nodes[v]['weight']
    print(f"Edge {i+1}: ({u} - {v}), Base Weight: {base_weight}, Node {u} Weight: {node_u_weight}, Node {v} Weight: {node_v_weight}")
